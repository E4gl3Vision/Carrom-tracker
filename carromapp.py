...
<style>
    ...
    /* Dialog custom styling for screenshots */
    div[role="dialog"] {
        border-radius: 16px;
    }
</style>
...
def save_match_callback():
    ...
    players_saved = 0
    st.session_state.last_match_summary = [] # Clear previous summary
    ...
    if players_saved > 0:
        ...
        # Trigger the pop-up modal to show
    ...
    if len(st.session_state.last_match_summary) > 0:
        ...
        for idx, p_data in enumerate(st.session_state.last_match_summary):
            with cols[idx]:
                ...
                st.metric("Final Rating", f"{p_data['Final_Rating']:.2f}", f"{p_data['Net_Coins']} Net Coins")
                st.write("---")
                ...
                for foul in penalties.keys():
                    if p_data[foul] > 0:
                        ...
                        fouls_exist = True
                if not fouls_exist: st.markdown("- *None*")
...
# Trigger Dialog if State is True
