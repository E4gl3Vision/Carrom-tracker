...
st.caption("Live multi-player dashboard with Net Coin tracking. Export seamlessly to Google Sheets.")
...
# Define which actions actually represent pocketing a coin (excluding streaks & failed red attempts)
...
for i, p in enumerate(players):
    ...
    with dash_cols[i]:
        # Include Net Coins in the scoreboard metric
        st.metric(
...
for i, p in enumerate(players):
    with tabs[i]:
        ...
        st.markdown(f"🪙 **Net Coins Pocketed:** `{net_c}` &nbsp;&nbsp; *(Total Pots: {p_c} | Total Fouls: {f_c})*")
        st.write("")
        ...
        # --- FIXED: Native Streamlit Toggle for Match Won ---
        ...
        if setup_cols[2].button(f"🧹 Reset Form", key=f"reset_{p}", use_container_width=True):
            for action in list(multipliers.keys()) + list(penalties.keys()):
                st.session_state[f"{p}_{action}"] = 0
            st.session_state[f"{p}_won"] = False # This automatically resets the toggle switch
            st.rerun()
        ...
        with col_pos:
            ...
            for idx, (action, weight) in enumerate(multipliers.items()):
                ...
                # Highlight coin pot actions vs streak/attempt actions
